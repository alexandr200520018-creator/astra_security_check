...
print_row() {
    ...
    local status_color=""
    case "$status" in
        "СООТВЕТСТВУЕТ") status_color="${GREEN}" ;;
        "НЕ СООТВЕТСТВУЕТ") status_color="${RED}" ;;
        *) status_color="${YELLOW}" ;;
    esac

    printf "${HEADER_COLOR}%s${NC}%s%s%s%s${status_color}%s${NC}\n" \
        "$name" "$file" "$param" "$current_value" "$expected_value" "$status"
}
...
print_tool_row() {
    ...
    echo "$name|astra-tool|$param|$current_value|$expected_value|$status" >> "$OUTPUT_FILE"
    ...
    printf "${HEADER_COLOR}%s${NC}%s%s%s${status_color}%s${NC}\n" \
        "$name" "$param" "$current_value" "$expected_value" "$status"
}
...
print_kernel_row() {
    ...
    echo "$name|kernel|$param|$current_value|$expected_value|$status" >> "$OUTPUT_FILE"
...
print_ssh_row() {
    ...
    echo "$name|ssh|$param|$current_value|$expected_value|$status" >> "$OUTPUT_FILE"
